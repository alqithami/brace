...
class ContainmentEnv(gym.Env):
    \"\"\"Step-level containment environment using a learned simulator.
    Action is MultiDiscrete: [target_idx (0..K-1), comms(0/1), soc(0/1), ir(0/1)].
    Observation is a compact vector:
      [t_norm, prev, growth, panic, budget_left_norm, hub_strength_mean, hub_strength_max]
    \"\"\"
    ...
    def __init__(self, step_stats: dict, sim_model, K=10, max_steps=15,
        ...
        super().__init__()
        self.step_stats = step_stats  # per cascade: arrays of per-step counts and hub strengths
        self.sim = sim_model
        self.K = K
    ...
    def _get_obs(self):
        ...
        return np.array([t_norm, prev, growth, panic, budget_left, hub_mean, hub_max], dtype=np.float32)
    ...
    def step(self, action):
        ...
        if c > self.budget_left:
            # enforce hard budget by cancelling action (no-op)
            comms = soc = ir = 0
            c = 0.0

        self.budget_left -= c

        # base simulator prediction: next new count proxy
        # features: t, cum_count proxy, new_count proxy, remaining_steps
        # we use prev as cum proxy; growth as new proxy
        ...
        # action effect scaled by targeted hub strength (0..1)
        ...
        # update prevalence: normalized proxy
        self.last_prev = self.prev
        self.prev = min(1.0, self.prev + next_growth/100.0)  # scale constant
        self.growth = min(1.0, next_growth/50.0)

        # panic dynamics: reduce with comms, increase slightly with spread
        ...
        truncated = False
        return self._get_obs(), float(r), terminated, truncated, {"cost": c, "violation": violation}

def build_step_stats(nodes: pd.DataFrame, edges: pd.DataFrame, panic: pd.DataFrame|None, K=10):
    # hub strengths based on out-degree within cascade (normalized)
    step_stats = {}
    for cid, g in nodes.groupby("cascade_id"):
        # compute hub strengths
...
def main():
    ...
    ap.add_argument("--sim-model", type=str, default=None, help="Path to trained simulator joblib (default: models/simulator/<ds>/sim_step_<split>.joblib)")
    ap.add_argument("--out", type=str, default="models/rl")
    ...
    edges["cascade_id"] = edges["cascade_id"].astype(str)
    ...
    step_stats = build_step_stats(nodes, edges, panic, K=10)
    ...
    model.save(str(out / f"ppo_{args.split}"))
    print("Saved PPO model to:", out / f"ppo_{args.split}")
